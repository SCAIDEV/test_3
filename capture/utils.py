...
def bottom_three_util(key):
    ...
    while True:
        ...
        while True:
            frame3 = rch.get_json('camera_original')
            if frame3 is not None:
                # print('Read the image')
                rch.set_json({'camera_original':None})
                break
            #print(datetime.now(),'Waiting for image in bottom two')

        #frame2 = rch.get_json('bottom_two_original')
        #print("KEY :      : :: :  : : : : :" , key)
...
consumer_mount_path = os.getcwd()+"/capture"
...
def get_inference_feed_url_util():
    ...
    feed_urls = ["http://localhost:8000/livis/v1/capture/bottom_three/camera_original/"]
    # dummmy_dct = {}
    ...
    # for i in ws_camera_dict:
        ...
        # dummmy_dct = {"camera_name":i['camera_name'] , "camera_url":url,"inspection_id":str(workstation_id),"camera_id":str(i['camera_id'])}
        # feed_urls.append(dummmy_dct)
    if feed_urls:
        return feed_urls
    else:
        return "NO FEED AVAILIABLE"
...
#         preprocessing_list = MongoHelper().getCollection(str(part_id) + PREPROCESSING_COLLECTION)
...
#             mp = MongoHelper().getCollection(part_id + DATASET_COLLECTION)
...
#                             experiments_list = MongoHelper().getCollection(str(part_id)+ EXPERIMENT_COLLECTION_BYPARTID)
